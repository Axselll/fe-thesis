	...
	return (
		<div className="flex flex-row rounded-lg h-full w-full bg-white bg-opacity-30 backdrop-filter backdrop-blur-lg p-4 items-center">
			{data?.commited?.map((item) => (
				<ol
					key={item._id}
					className="border-l md:border-l-0 md:border-t border-gray-300 md:flex md:justify-center md:gap-6">
					<li>
						<div className="flex md:block flex-start items-center pt-2 md:pt-0">
							...
							<p className="text-gray-500 text-sm mt-2 mr-5">
								Created on {moment(item.createdAt).format("LL")}
							</p>
						</div>
						<div className="mt-0.5 ml-4 md:ml-0 pb-5">
							<h4 className="text-gray-800 font-semibold text-xl mb-1.5 mr-5">
								{item.title}
							</h4>
